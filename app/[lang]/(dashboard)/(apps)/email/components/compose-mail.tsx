    ...
    >
      <CardHeader className="border-none p-0 block mb-0">
        <div className="py-3.5 px-6 bg-primary/10 rounded-t-md flex gap-3">
          <div className="text-sm font-medium text-default-900 flex-1">New Message</div>
          <button type="button" className="flex-none" onClick={handleMinimize}>
            <Minus className="h-5 w-5 text-default-500" />
          </button>
          <button type="button" className="flex-none hidden lg:block" onClick={handleMaximize}>
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="relative h-11 border-b border-default-200 flex items-center ">
            <label htmlFor="recepients" className="text-sm text-default-500 cursor-text">
              {true ? 'To' : 'Recepients'}
            </label>
            <input
              {...register('name')}
          ...
          <div className="snow-editor border border-default-200 h-[250px]" ref={quillRef}></div>
