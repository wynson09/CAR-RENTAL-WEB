  ...
  return (
    <>
      <ul>
        {taskFilters.map((item, index) => (
          ...
          >
            <Icon
              icon={item.icon}
              className={cn('w-4 h-4 text-default-600 group-hover:text-primary', '')}
            />
            <span
              className={cn('text-sm font-medium text-default-600 group-hover:text-primary', '')}
            >
              {item.label}
            </span>
          </li>
      ...
      <div className="mt-3 md:px-4 px-2">
        <div className="text-xs font-medium text-default-800 uppercase">Priority</div>
        <ul className="mt-3">
          {priorityFilters.map((item, index) => (
            ...
            >
              ...
              <span className="text-sm font-medium text-default-600">{item.total}</span>
            </li>
          ))}
        </ul>
      </div>
      <div className="border border-dashed mt-5 border-default-300"></div>
      <div className="text-xs font-medium text-default-800 uppercase my-3 px-4"> Chats</div>
