...
interface StepProps extends React.HTMLAttributes<HTMLLIElement>, VariantProps<typeof stepVariants> {
  children?: React.ReactNode;
...
const Step = React.forwardRef<HTMLLIElement, StepProps>(
  (
    ...
    const hasSpecificProps =
      gap !== undefined || alternativeLabel !== undefined || direction !== undefined;
    // has render
    const renderChildren = !isContentRight || (isContentRight && !hasSpecificProps);
    return (
      <li
        ref={ref}
      ...
      >
        <div
          ...
          className={cn('flex items-center flex-row relative z-[1] ', {
            ...
            'before:w-[calc(100%-85px)]': alternativeLabel && gap && isBeforeLast,
        ...
        >
          ...
          {isContentRight && renderChildren && <div className=" bg-card px-3">{children}</div>}
