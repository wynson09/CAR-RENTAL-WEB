...
const progressVariants = cva('relative overflow-hidden rounded-full bg-default-200', {
  variants: {
    color: {
...
const circleVariants = cva('[&_[path-color]]:text-default-200 ', {
  variants: {
    color: {
      ...
      destructive: '[&_[bar-color]]:text-destructive [&_[text-color]]:fill-destructive',
...
const Progress = React.forwardRef<React.ElementRef<typeof ProgressPrimitive.Root>, ProgressProps>(
  ({ className, value, color, size, showValue, isStripe, isAnimate, ...props }, ref) => {
...
>(
  (
    { className, value, color, size, showValue, loading, customContent, isStripe, ...props },
    ref
  ) => {
    const radius = 40;
