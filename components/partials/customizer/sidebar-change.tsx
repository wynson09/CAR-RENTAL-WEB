...
const SidebarChange = () => {
  ...
  return (
    ...
    >
      ...
      <div className="text-muted-foreground font-normal text-xs mb-4">Choose your layout</div>
      <div className=" grid grid-cols-3 gap-3">
        {sidebarOptions.map((sidebarOption) => (
          <div key={sidebarOption.key}>
            <button
              onClick={() => setSidebarType(sidebarOption.key)}
              disabled={sidebarOption.disabled && sidebarOption.disabled(layout)}
              className={cn(
                ' border block  rounded relative h-[72px] w-full disabled:cursor-not-allowed',
                {
                  'text-[--theme-primary] border-[--theme-primary]':
                    sidebarType === sidebarOption.key,
                  'text-muted-foreground border-border': sidebarType !== sidebarOption.key,
                }
              )}
            >
              {sidebarType === sidebarOption.key && (
